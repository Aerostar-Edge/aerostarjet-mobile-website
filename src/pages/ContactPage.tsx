    ...
    <PageLayout>
      ...
      <section className="space-y-6 bg-bg-grey px-4 py-10">
        <div className="space-y-2 text-center">
          ...
          <p className="text-description px-[41px] text-muted-alt">
            Tell us a little about yourself and our counsellors will reach out to guide you.
          </p>
        </div>
        <form
          className="space-y-4 rounded-lg border border-border-alt bg-surface p-6 shadow-md"
      ...
      <section className="space-y-6 bg-bg-light px-4 py-10">
        ...
        <div className="space-y-4">
          {branches.map((branch) => (
            <div key={branch.city} className="rounded-2xl border border-border bg-surface p-6 shadow-md">
              <h3 className="font-bold text-navy">{branch.city}</h3>
              <p className="text-description mt-2 text-muted">{branch.address}</p>
              <a
                href={`tel:${branch.phone.replace(/\s/g, '')}`}
                className="mt-2 flex min-h-12 items-center text-xs font-semibold text-primary"
              >
                {branch.phone}
              </a>
            </div>
